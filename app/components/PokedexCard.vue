...
<template>
    ...
    >
        <div
            ...
            :class="pokemonTypeColors[`${props.pokemon.types?.[0]?.name ?? 'normal'}Card` as keyof typeof pokemonTypeColors]"
        >
            <img
        ...
        <div class="space-y-3 p-4">
            <div class="flex items-center justify-between text-sm text-slate-400">
                ...
                <span class="rounded-full bg-slate-800/70 px-2 py-1 text-[10px] uppercase tracking-wide">{{ props.pokemon.types?.[0]?.name ?? 'normal' }}</span>
            </div>
